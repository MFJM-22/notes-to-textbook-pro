...
const statusLabel: Record<string, { label: string; icon: React.ComponentType<{ className?: string }>; color: string }> = {
  ...
  return (
    <div>
      <div className="flex items-end justify-between">
        ...
        <Link to="/books/new" className="btn-primary btn-primary-hover">
        ...
        {isLoading ? (
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {books.map((b) => {
              ...
              const Icon = s.icon;
              return (
                <Link key={b.id} to="/books/$id" params={{ id: b.id }} className="book-card book-card-hover block">
                  <div className="flex items-start justify-between">
                    <BookOpen className="h-6 w-6" style={{ color: "var(--gold)" }} />
                    <span className="inline-flex items-center gap-1 rounded-full border border-border px-2 py-0.5 text-xs" style={{ color: s.color }}>
...
function EmptyState() {
  return (
    <div className="book-card grid place-items-center py-16 text-center">
      ...
      <Link to="/books/new" className="btn-primary btn-primary-hover mt-6">
